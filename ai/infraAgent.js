...
const { pool } = require('../backend/infra/proxy/proxyPool');
const { generateFingerprint, toPlaywrightContext, toAxiosHeaders } = require('../backend/infra/proxy/fingerprint');
...
class InfraAgent {
    ...
    getScrapingContext(options = {}) {
        ...
        return {
            ...
            playwrightConfig: proxy ? {
                ...
                proxy: proxyUrl ? require('../backend/infra/proxy/proxyPool').ProxyPool.parseProxyUrl(proxyUrl) : undefined,
            } : toPlaywrightContext(fingerprint),
        };
    }
    ...
    async getBrowserSession(options = {}) {
        ...
        if (ctx.proxyUrl) {
            launchOptions.proxy = require('../backend/infra/proxy/proxyPool').ProxyPool.parseProxyUrl(ctx.proxyUrl);
        }
