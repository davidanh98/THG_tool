...
const { canAct } = require('../../backend/core/rateLimiter');
...
async function phaseBroadcast(browser, accounts) {
    ...
    try {
        ...
        try {
            const groupDiscovery = require('../agents/groupDiscovery');
...
async function phaseScrape() {
    ...
    try {
        const { scrapeFacebookGroups } = require('../../backend/infra/scraper/orchestrator');
