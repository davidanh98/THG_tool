...
async function classifyPosts(posts) {
    ...
    for (const post of posts) {
        ...
        if (DOMESTIC_VN_REGEX.test(content) || IRRELEVANT_REGEX.test(content) || IMPORT_VN_REGEX.test(content)) {
            console.log(`[Sieve] 🚫 Discarded (Domestic/Irrelevant/Import): ${content.substring(0, 50)}`);
            results.push({ ...post, recommended_lane: 'discard', is_relevant: false, reason_summary: 'Domestic, Irrelevant, or Import' });
            continue;
        }
